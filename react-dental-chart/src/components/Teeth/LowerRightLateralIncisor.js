...
const RightLowerLateralIncisor = ({ ...props }) => {
  const group = useRef()

  const { nodes, materials } = useGLTF('/assets/Right_Lower_Lateral_Incisor.glb')

  return (
    <group ref={group} {...props} dispose={null}>
      <group position={[0, -1.63, 0]} rotation={[Math.PI / 2, 0, 0.09]} scale={[-0.76, 0.76, 0.76]}>
        <mesh
    ...
    <>
      <Canvas>
        ...
        <Suspense fallback={null}>
          <RightLowerLateralIncisor />
        </Suspense>
      </Canvas>
      <div>Clicked on lower right lateral incisor</div>
    </>
  )
}
