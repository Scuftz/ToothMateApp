...
const RightLowerSecondMolar = ({ ...props }) => {
  const group = useRef()

  const { nodes, materials } = useGLTF('/assets/Right_Lower_Second_Molar.glb')
    ...
    <>
      <Canvas>
        ...
        <Suspense fallback={null}>
          <RightLowerSecondMolar />
        </Suspense>
      </Canvas>
      <div>Clicked on lower right second molar</div>
    </>
  )
}
