...
const RightLowerSecondPremolar = ({ ...props }) => {
  const group = useRef()

  const { nodes, materials } = useGLTF('/assets/Right_Lower_Second_Premolar.glb')

  return (
    <group ref={group} {...props} dispose={null}>
      <group position={[0, -1.61, 0]} rotation={[Math.PI / 2, 0, 0]} scale={[-0.54, 0.54, 0.54]}>
        <mesh
