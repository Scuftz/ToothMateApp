...
const UpdateClinicScreen = () => {
  const { state: authState, updateUserClinic } = useContext(AuthContext);
  ...
  return (
    <LinearGradient colors={['#78d0f5', 'white', '#78d0f5']} style={styles.container}>
      <View style={styles.container}>
        <Text style={{ fontSize: 50, alignSelf: 'center', color: '#000', fontFamily: 'Righteous_400Regular' }}>
          {' '}
          ToothMate{' '}
        </Text>
        ...
        />
        {authState.errorMessage && <Text style={styles.errorMessage}>{authState.errorMessage}</Text>}
        <Spacer>
          <Button
